...
class TestTransactionReader():
    ...
    def test_different_date_fields(self):
        reader_bookingdate = pytest.helpers.get_transactions_petstore(date_field = ['bookingdate'])
        reader_analysisdate = pytest.helpers.get_transactions_petstore(date_field = ['analysisdate','transactiondate'])
